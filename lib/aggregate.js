...
import { calcCostAt, costWithPrice, resolvePrice } from './pricing.js';

// 峰谷分桶：peak=高峰价 / offpeak=低谷（闲时）价 / flat=未启用峰谷的单一价。
const emptyTiers = () => ({
  peak: { token: 0, cost: 0 },
  offpeak: { token: 0, cost: 0 },
  flat: { token: 0, cost: 0 },
});
...
function emptyRow(key, displayName, configured) {
  return {
    ...
    // 峰谷分桶（见 emptyTiers）与「本行是否按峰谷时段计过价」标记，供 UI 展示峰谷拆分
    tiers: emptyTiers(),
    tou: false,
    _cost: 0, // 逐条累计的花费（元），区别于按合计 token 反推的单一价口径
    _priced: false,
  };
}
...
// 单条记录 → 计价用量口径（缓存写归入 input）；聚合与 UI 共用，避免各处重复拼装。
...
// 单条记录按已解析单价计费
const costOf = (p, r) => costWithPrice(p, recordUsage(r));

// 逐条累计：把一条记录的花费与 token 计入对应时段分桶（未定价 → 不动，保持 cost=null 语义）
function addTier(row, p, r, tokens) {
  const c = costOf(p, r);
  const b = row.tiers[p.kind] || row.tiers.flat;
  b.token += tokens;
  b.cost += c;
  row._cost += c;
  row._priced = true;
  if (p.tou) row.tou = true;
  return c;
}

// priceOverrides 为可选价目表覆盖（缺省用内置默认价目表），用于支持用户自定义单价。
// 计价支持峰谷：每条记录按其发生时间（r.time）取当时单价后逐条累计，
// 因此同一模型的峰/谷记录不会被平均成单一价；行内 tiers 给出峰/谷/单一价各自的 token 与花费。
// 每行含 cost = 估算花费（元，= 各时段花费之和），未定价模型为 null。
export function aggregateByModel(records, configModels, range, priceOverrides = null) {
  ...
  for (const r of records) {
    ...
    // 峰谷计价：取该条记录发生时刻的生效单价（未启用峰谷的模型等价于单一价）
    const p = resolvePrice(row.key, priceOverrides, r.time);
    if (p) addTier(row, p, r, r.inputOther + r.inputCacheRead + r.inputCacheCreation + r.output);
  ...
  for (const row of rows.values()) {
    ...
    // 估算花费：逐条按当时单价累加（见 addTier）。
    // 零用量行（requests===0）或全部记录未定价 → cost 置 null → 显示 —，避免误读成 ¥0.00。
    row.cost = row.requests === 0 || !row._priced ? null : row._cost;
  }
  return [...rows.values()].sort((a, b) => b.total - a.total);
}

// 汇总卡：对 aggregateByModel 的结果（通常已按时间段过滤）求总计
// 除原有字段外，另返回 tier = { peak|offpeak|flat: {token, cost} } 与 tou（是否含峰谷计价数据），供峰谷汇总展示。
export function summarize(rows) {
  let total = 0, output = 0, requests = 0, sumStreamMs = 0, hit = 0, miss = 0, cost = 0;
  const tier = emptyTiers();
  let tou = false;
  for (const r of rows) {
    ...
    if (r.tou) tou = true;
    if (r.tiers) {
      for (const k of ['peak', 'offpeak', 'flat']) {
        const src = r.tiers[k];
        if (!src) continue;
        tier[k].token += src.token || 0;
        tier[k].cost += src.cost || 0;
      }
    }
  }
  return {
    total, output, requests, cost, tier, tou,
...
// 单条记录的估算花费（元）；未定价模型/null。
// 口径与 aggregateByModel 一致：缓存写归入 input，并按记录发生时间命中峰谷时段单价。
function recordCost(key, r, priceOverrides) {
  return calcCostAt(key, recordUsage(r), priceOverrides, r.time);
}
...
// 花费逐条按发生时间取价累计（支持峰谷）；未定价模型 cost=null，不计入总 cost。
export function interruptStats(records, markers, configModels, range, priceOverrides = null) {
  ...
  for (const r of records) {
    ...
    let row = rows.get(key);
    if (!row) {
      row = { model: shortKey(r.model), key, totalToken: 0, count: 0, _input: 0, _cacheRead: 0, _cacheCreation: 0, _output: 0, _cost: 0, _priced: false, _tou: false };
      rows.set(key, row);
    }
    ...
    const p = resolvePrice(row.key, priceOverrides, r.time);
    if (p) {
      row._cost += costOf(p, r);
      row._priced = true;
      if (p.tou) row._tou = true;
    }
    total += r.inputOther + r.inputCacheRead + r.inputCacheCreation + r.output;
    requests += 1;
  }
  const byModel = [...rows.values()].map((row) => {
    const cost = row._priced ? row._cost : null;
    return { model: row.model, key: row.key, totalToken: row.totalToken, cost, count: row.count, tou: row._tou };
